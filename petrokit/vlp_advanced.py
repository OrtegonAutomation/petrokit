...
def vlp_curve_hagedorn(q_liq_range_stb_d: Iterable[float],
    ...
    H_arr = []
    for q in q_range:
        dp, H = hagedorn_brown_dp(q, q_gas_mscf_d, d_in, well_length_ft, theta_deg,
                                  rho_l, rho_g, mu_l, mu_g, roughness, f_guess)
        pwf_arr.append(dp)
        H_arr.append(H)
    return q_range, np.array(pwf_arr), np.array(H_arr)
...
def plot_vlp_compare(q_liq_range, q_gas, d_in, L_ft, theta_deg, rho_l, rho_g, mu_l, mu_g, f_guess=None):
    q_arr, pwf_bb, H_bb = vlp_curve_beggs(
        q_liq_range_stb_d=q_liq_range, 
        q_gas_mscf_d=q_gas, 
        d_in=d_in, 
        well_length_ft=L_ft, 
        theta_deg=theta_deg, 
    ...
    _, pwf_hb, H_hb = vlp_curve_hagedorn(
        q_liq_range_stb_d=q_liq_range,
        q_gas_mscf_d=q_gas,
        d_in=d_in,
        well_length_ft=L_ft,
