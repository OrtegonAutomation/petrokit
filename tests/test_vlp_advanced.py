...
def test_beggs_brill_positive_and_holdup():
    dp, H = beggs_brill_dp(500, 200, 3.5, 8000, 90, 60, 0.08, 1.0, 0.02)
    assert dp >= 0
    assert 0 < H < 1

def test_hagedorn_brown_positive_and_holdup():
    dp, H = hagedorn_brown_dp(500, 200, 3.5, 8000, 60, 0.08, 1.0, 0.02, 90)
    assert dp >= 0
    assert 0 < H < 1
...
def test_vlp_curves_lengths():
    ...
    q2, pwf_hb, H_hb = vlp_curve_hagedorn(q_range, 200, 3.5, 8000, 60, 0.08, 1.0, 0.02)
